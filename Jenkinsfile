...
cfg = jplConfig('gp-nginx', 'bash', '', [email: env.CI_NOTIFY_EMAIL_TARGETS])
...
def buildAndPublishDockerImage(nextReleaseNumber = "") {
    ...
    docker.withRegistry("", 'docker-token') {
        def customImage = docker.build("${env.DOCKER_ORGANIZATION}/${cfg.projectName}:${nextReleaseNumber}", "--pull --no-cache statics")
        customImage.push()
